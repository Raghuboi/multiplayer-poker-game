...
function Game(props) {
    ...
    useEffect(() => {
        socket.on('initGameState', ({ gameOver, winner, turn, player1Deck, player2Deck, houseDeck, increment, player1Chips, player2Chips, numberOfTurns, pot 
        , raiseAmount, player1Name, player2Name}) => {
    ...
    return (
            <div className="game-bg noselect">
                <div className="game-board">
                    <Box 
                        position="absolute" top="0.3125rem" left="0.3125rem" translate="transform(-0.3125rem, -0.3125rem)"
                        backgroundColor="whitesmoke" color="black" padding="0.5rem" borderRadius="0.25rem"
                    >
                        <Heading size="md" fontFamily="inherit" >Room Code: {data.roomCode}</Heading>
                    </Box>                                   
                    <Cards
                    numberOfTurns={numberOfTurns}
                    player1Deck={player1Deck}
                    ...
                    >
                        ...
                        maxRaise={currentUser === 'Player 1' ? player1Chips : player2Chips}
...
/**
 *             {houseDeck && gameOver===false && numberOfTurns>2 && <Heading size="md" fontFamily="inherit">
                Hand: {(currentUser==='Player 1') ? getHand(player1Deck, houseDeck).type : getHand(player2Deck, houseDeck).type}
            </Heading>}

                        <Heading color="white" size="md" fontFamily="inherit">
                {(currentUser===turn) ? "Your Turn" : (turn==='Player 1') ? `${player1Name}'s Turn` : `${player2Name}'s Turn`}
            </Heading>

                    {gameOver===true && <div className="end-screen">
            <h1>Winner: {winner==='' && getWinner(player1Name, player2Name, getHand(player1Deck, houseDeck), getHand(player2Deck, houseDeck)) || winner}</h1>
            <h1>{player1Name}'s Hand: {getHand(player1Deck, houseDeck).type}</h1>
            <h1>{player2Name}'s Hand: {getHand(player2Deck, houseDeck).type}</h1>
            <button onClick={()=>{setRestart(true)}}>Restart</button>
            </div>}

            <div className="info"><h3 className="info-item">Room Code: {data.roomCode}</h3></div>
 * 
 */
