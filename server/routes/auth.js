...
router.get('/', allowCrossDomain, verifyJWT, async (req, res) => {
	res.status(401)
})

router.post('/signin', allowCrossDomain, verifyJWT, async (req, res, next) => {
	...
	try {
		...
		if (!user) res.status(401).json({ error: 'Not found' })
			...
			if (isValid) {
				...
				res.cookie('access', accessToken, ACCESS_COOKIE_OPTIONS)
				res.json({
					user: { id: user._user, email: user.email, username: user.username },
					message: 'Signed in',
				})
...
router.post('/signup', allowCrossDomain, verifyJWT, async (req, res, next) => {
	...
	try {
		...
		if (usernameExists)
			...
			res
				.status(200)
				.json({ message: 'Registered, verify your email to Sign In' })
		}
	} catch (e) {
		console.error(e)
		res.status(500).json({ error: e.message })
...
router.post('/signout', allowCrossDomain, async (req, res, next) => {
